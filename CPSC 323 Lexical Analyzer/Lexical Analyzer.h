...
protected:						 /* INTEGER,   REAL,      OPERATOR,  STRING,    UNKNOWN,   SPACE,   COMMENT,  SEPARATOR, DOLLAR */
	...
	/* STATE 2 */		{REAL,      REAL,      UNKNOWN,   REJECT,	 REJECT,	REJECT,    REJECT,  COMMENT,  UNKNOWN,	 REJECT},
	/* STATE 3 */		{OPERATOR,  REJECT,	   REJECT,	  REJECT,	 REJECT,    REJECT,    REJECT,  COMMENT,  REJECT,	 REJECT},
	/* STATE 4 */		{STRING,    STRING,    REJECT,    REJECT,    STRING,    UNKNOWN,   REJECT,  COMMENT,  REJECT,	 STRING},
...
int FSM::getCol(int prevState, char currChar) {
	if (isSpace(currChar))
		return SPACE;
	...
	else if (isReal(prevState, currChar))
		return REAL;
...
bool FSM::isReal(int prevState, char currChar) {
	if (prevState == INTEGER && currChar == '.')
		return true;
	return false;
}
